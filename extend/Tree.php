...
class Tree {
    ...
    public function get_tree_list($myid, $adds='') {
        $number = 1;
        //一级栏目
        $child = $this->get_child($myid);
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                $this->ret[] = $value;
                $nbsp = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
                $this->get_tree_list($value['id'], $adds . $k . $nbsp);
                $number++;
            }
        }
        return $this->ret;
    ...
    public function get_tree_array_2($myid, $str, $sid = 0, $adds = '', $str_group = '') {
        $retarray = array();
        //一级栏目数组
        $child = $this->get_child($myid);
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                @extract($value);
                $value["child"] = $this->get_tree_array_2($id, '');
                $retarray[] = $value;
            }
        }
        return $retarray;
    ...
    public function get_tree_array_3($myid, $str, $sid = 0, $adds = '', $str_group = '') {
        $retarray = array();
        //一级栏目数组
        $child = $this->get_child($myid);
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                @extract($value);
                $value["children"] = $this->get_tree_array_3($id, '');
                $retarray[] = $value;
            }
        }
        return $retarray;
